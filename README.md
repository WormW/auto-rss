...
- **订阅与下载**
  - 创建、编辑、删除、启用/停用订阅。
  - 一个订阅可配置多个平等的 RSS feed；每个 feed 独立设置字幕组、集数偏移、启用状态、水位线和健康状态。
  - 多个 feed 同时发现同一相对集数时，第一个成功受理的资源进入下载，后到的不同资源进入人工候选，不按列表顺序设置优先级。
  - 新增 feed 或修改 URL/偏移后，首次成功同步只建立基线并记录历史候选，不自动下载 RSS 中的历史缺集。
  - 订阅预览、手动采集缺失剧集、合集种子下载、批量导入、导入/导出订阅。
  - RSS 定时刷新、手动全局刷新、增量水位线、种子 Hash 去重、同集资源候选与人工替换。
  - 剧集台账、批量状态管理、RSS 换源基线对账，以及可恢复的人工资源替换。
...
- **组织与筛选**
  - feed 级字幕组和集数偏移；订阅级语言、总集数、包含/排除关键词和过滤规则。
