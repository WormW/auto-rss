...
type JWTService interface {
	...
	// ValidateAccessToken 验证access token (返回 middleware.TokenClaims 以兼容接口)
	ValidateAccessToken(tokenString string) (*middleware.TokenClaims, error)
...
func (s *jwtService) ValidateAccessToken(tokenString string) (*middleware.TokenClaims, error) {
	...
	if claims, ok := token.Claims.(*Claims); ok && token.Valid {
		...
		return &middleware.TokenClaims{
			UserID:    userID,
			Username:  claims.UserID, // Use UserID string as username fallback
